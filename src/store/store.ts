...
export const useStore = create<StoreState>((setState, getState) => {
  const scheduleHash = () => {
    ...
    hashTimer = setTimeout(() => {
      const { structureVersion, lutVersion, activePreset, set, applyPreset, bulkSet, ...params } =
        getState()
  ...
  return {
    ...
    applyPreset: (name) => {
      const patch = { ...DEFAULT_PARAMS, ...PRESETS[name] }
      setState({ ...patch, activePreset: name } as Partial<StoreState>)
      setState((s) => ({
